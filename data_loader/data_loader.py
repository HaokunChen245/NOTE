...
def split_data(
    entire_data, valid_split, test_split, train_max_rows, valid_max_rows, test_max_rows
):
    ...
    train_data, valid_data, test_data = keep_order_split(
        entire_data, train_size, valid_size, test_size
    )
...
def datasets_to_dataloader(
    datasets, batch_size, concat=True, shuffle=True, drop_last=False
):
    if concat:
        data_loader = None
        if len(datasets):
            if type(datasets) is torch.utils.data.dataset.Subset:
                datasets = [datasets]
            if (
                sum([len(dataset) for dataset in datasets]) > 0
            ):  # at least one dataset has data
...
def domain_data_loader(
    ...
    if domains is not None:
        ...
        if conf.args.validation:
            processed_domains = sorted(
                list(set(conf.args.opt["src_domains"]) - set([conf.args.tgt]))
            )  # Leave-one-user-out
        else:
            processed_domains = conf.args.opt["src_domains"]
    else:
    ...
    if dataset in ["cifar10"]:
        ...
        loaded_data = load_cache(
            dataset, processed_domains, file_path, transform=transform
        )

        if not loaded_data:
            loaded_data = CIFAR10Dataset(
                file=file_path, domains=cond, max_source=num_source, transform=transform
            )
            save_cache(
                loaded_data, dataset, processed_domains, file_path, transform=transform
            )
    ...
    for train_data in entire_datasets:
        ...
        print(
            "#Multi?:{:d} data_loader len:{:d} Train: {:d}\t# Valid: {:d}\t# Test: {:d}".format(
                True if domains == ["rest"] else False,
                total_len,
                len(train_data),
                len(valid_data),
                len(test_data),
    ...
    if is_src:
        train_data_loader = datasets_to_dataloader(
            train_datasets,
            batch_size=batch_size,
            concat=True,
            drop_last=True,
        ...
        train_data_loader = datasets_to_dataloader(
            train_datasets, batch_size=1, concat=True, drop_last=False, shuffle=False
        )
    valid_data_loader = datasets_to_dataloader(
        valid_datasets, batch_size=batch_size, concat=True, shuffle=False
    )
    test_data_loader = datasets_to_dataloader(
        test_datasets, batch_size=batch_size, concat=True, shuffle=False
    )
    ...
    data_loader = {
        ...
        "num_domains": sum(
            [dataset.dataset.get_num_domains() for dataset in train_datasets]
        ),
    }

...
class DNN:
    def __init__(self, model, source_dataloader, target_dataloader, write_path):
        ...
        if (
            conf.args.dataset in ["cifar10", "cifar100"]
            and conf.args.tgt_train_dist == 0
        ):
            self.tgt_train_dist = (
                4  # Dirichlet is default for non-real-distribution data
            )
        else:
        ...
        if conf.args.method == "Src":
            if conf.args.dataset in [
                ...
                self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                    self.optimizer,
                    T_max=conf.args.epoch * len(self.source_dataloader["train"]),
                )
    ...
    def target_data_processing(self):
        ...
        if self.tgt_train_dist == 0:
            num_samples = (
                conf.args.nsample
                if conf.args.nsample < len(features)
                else len(features)
            )
        ...
        remainder = (
            len(result_feats) % conf.args.update_every_x
    ...
    def log_accuracy_results(self, condition, suffix, epoch, cm_class):
        ...
        print(
            "[epoch:{:d}] {:s} {:s} class acc: {:.3f}".format(
                epoch, condition, suffix, class_accuracy
            )
        )
    ...
    def train(self, epoch):
        ...
        if conf.args.method in ["Src", "Src_Tgt"]:
            ...
            for batch_idx, labeled_data in tqdm(
                enumerate(self.source_dataloader["train"]), total=num_iter
            ):
                feats, cls, _ = labeled_data
                feats, cls = feats.to(device), cls.to(device)

                if (
                    torch.isnan(feats).any() or torch.isinf(feats).any()
                ):  # For reallifehar debugging
                    print("invalid input detected at iteration ", batch_idx)
                    exit(1)
                # compute the feature
                preds = self.net(feats)
                if (
                    torch.isnan(preds).any() or torch.isinf(preds).any()
                ):  # For reallifehar debugging
                    print("invalid input detected at iteration ", batch_idx)
                    exit(1)
                ...
                if (
                    torch.isnan(class_loss).any() or torch.isinf(class_loss).any()
                ):  # For reallifehar debugging
                    print("invalid input detected at iteration ", batch_idx)
                    exit(1)
    ...
    def evaluation(self, epoch, condition):
        ...
        with torch.no_grad():
            ...
            class_cm_test_data = confusion_matrix(
                tgt_cls.cpu(), y_pred.cpu(), labels=labels
            )
        ...
        class_accuracy = (
            100.0 * np.sum(np.diagonal(class_cm_test_data)) / np.sum(class_cm_test_data)
        )
        print(
            "[epoch:{:d}] {:s} {:s} class acc: {:.3f}".format(
                epoch, condition, "test", class_accuracy
            )
        )
    ...
    def evaluation_online(self, epoch, condition, current_samples):
        ...
        with torch.no_grad():
            ...
            progress_checkpoint = [
                int(i * (len(self.target_train_set[0]) / 100.0)) for i in range(1, 101)
            ]
            for i in range(
